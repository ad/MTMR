...
class CurrencyBarItem: CustomButtonTouchBarItem {
    ...
    func setCurrency(value: Float32) {
        ...
        let regularFont = self.attributedTitle.attribute(NSAttributedString.Key.font, at: 0, effectiveRange: nil) as? NSFont ?? NSFont.systemFont(ofSize: 15)
        let newTitle = NSMutableAttributedString(string: title as String, attributes: [NSAttributedString.Key.foregroundColor: color, NSAttributedString.Key.font: regularFont])
