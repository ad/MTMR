...
class TouchBarController: NSObject, NSTouchBarDelegate {
    ...
    func reloadPreset(path: String) {
        lastPresetPath = path
        let items = path.fileData?.barItemDefinitions() ?? [BarItemDefinition(type: .staticButton(title: "bad preset"), action: .none, tapAction: TapAction(actionType: TapActionType.none), longTapAction: LongTapAction(actionType: TapActionType.none), additionalParameters: [:])]
    ...
    func createItem(forIdentifier identifier: NSTouchBarItem.Identifier, definition item: BarItemDefinition) -> NSTouchBarItem? {
        ...
        case .cpu(interval: let interval):
        ...
        if let longTapAction = self.longTapAction(forItem: item), let item = barItem as? CustomButtonTouchBarItem {
            item.longTapClosure = longTapAction
        }
